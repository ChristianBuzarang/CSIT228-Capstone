 ...
 * DataStore
 *
 * In-memory data source for all GymQuest entities.
 * Acts as a stand-in for a database until SQLite/MySQL is wired in.
 *
 * ── Changes from original ──────────────────────────────────────────────────
 *  • {@link #getInstance()} is now {@code synchronized} (thread-safety fix).
 *  • {@link #buildWorkouts()} now uses the full 9-argument {@link Workout}
 *    constructor to populate {@code category}, {@code description}, and
 *    {@code imagePath} on every workout — required by {@link WorkoutService}
 *    and the enhanced card / detail views.
 */
...
public class DataStore {
     ...
     * {@code synchronized} prevents a race condition on the first call.
     */
