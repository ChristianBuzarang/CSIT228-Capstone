...
import com.gymquest.model.WorkoutService;
 ...
 * WorkoutsView
 *
 * Displays the full GymQuest workout catalog in a responsive 3-column grid.
 *
 * ── Changes from original ──────────────────────────────────────────────────
 *  • Search bar — live-filters cards as the user types (keyword matches title
 *    or description via {@link WorkoutService#searchAndFilter}).
 *  • Category filter pills — "All" + one pill per {@link WorkoutCategory}.
 *    Clicking a pill narrows the grid; clicking the active pill again resets.
 *  • WorkoutService wired in — replaces the direct DataStore call so all
 *    search/filter logic lives in the service, not the view.
 *  • GridPane ColumnConstraints fixed — cards now stretch to fill their column
 *    (the original code used {@code GridPane.setHgrow} which has no effect
 *    without explicit ColumnConstraints).
 *  • Empty-state label shown when no workouts match the current query.
 */
